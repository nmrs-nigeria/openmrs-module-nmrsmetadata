package org.openmrs.module.nmrsmetadata.fragment;
...
/**
 * Overrides the mapping to the login page for the reference application to a a customized page for
 * UgandaEMR
 */
@Component
public class CustomAppUiHeaderFragmentRequestMapper implements FragmentRequestMapper {
	...
	public boolean mapRequest(FragmentRequest request) {
		if (request.getProviderName().equals("appui")) {
			if (request.getFragmentId().equals("header")) {
				// change to the custom header provided by the module
				request.setProviderNameOverride("nmrsmetadata");
				request.setFragmentIdOverride("customAppUiHeader");
